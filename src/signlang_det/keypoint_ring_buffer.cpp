...
namespace signlang::signlang_det {
  ...
  auto compute_buffer_capacity(std::uint32_t sequence_length, float overlap_ratio) -> std::uint32_t {
    return compute_buffer_capacity_impl(sequence_length, overlap_ratio);
  }

  KeypointRingBuffer::KeypointRingBuffer(std::uint32_t capacity) : buffer_(capacity), capacity_(capacity) {
    if (capacity == 0) {
  ...
  auto KeypointRingBuffer::copy_latest_window_locked(std::uint32_t window_size) const -> std::vector<FeatureVector> {
  ...
  void KeypointRingBuffer::notify_stop() { changed_.notify_all(); }

  auto KeypointRingBuffer::capacity() const -> std::uint32_t { return capacity_; }
